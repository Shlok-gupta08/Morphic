...
import { extractPages as apiExtractPages, saveFile, downloadBlob } from '@/services/api';
...
const handle = async () => {
  if (state.files.length === 0 || !state.pages.trim()) return;
  setStatus('processing');
  setError('');
  try {
    const pageNums = state.pages.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n));
    const blob = await apiExtractPages(state.files[0], JSON.stringify(pageNums));

    const newResult: PreviewResult = {
      id: crypto.randomUUID(),
      name: `extracted-${state.files[0].name}`,
      blob
    };
      ...
      <div className="w-full space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={state.files.length === 0 || !state.pages.trim() || status === 'processing'}
          >
            {status === 'processing' ? 'Extracting...' : (activeResult ? 'Extract Again' : 'Extract Pages')}
          </button>
  ...
  >
    ...
    {state.files.length > 0 && (
      <div className="p-6 rounded-2xl bg-surface-100 border border-surface-300 space-y-5">
        <div>
          <label className="text-xs font-medium text-ink-muted uppercase tracking-wider mb-2 block">Pages to extract</label>
          <input
            type="text"
            placeholder="e.g. 1, 3, 5, 7"
            value={state.pages}
            onChange={e => setState(prev => ({ ...prev, pages: e.target.value }))}
            className="input-field"
          />
          <p className="text-[11px] text-ink-faint mt-1">Comma-separated page numbers</p>
        </div>
      </div>
    )}
